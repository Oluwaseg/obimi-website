...
/**
 * Knowledge Hub Category Page Component
 * Built with Next.js and Tailwind CSS v4
 *
 * Design Concept:
 * - Scannable article list with clean cards.
 * - Minimalist header with breadcrumbs.
 * - Integrated search and filter for quick navigation.
 * - Focus on readability and quick access.
 */
...
export function KnowledgeHubCategory({ category }: CategoryPageProps) {
      ...
      <section className='max-w-[1440px] mx-auto px-6 md:px-12 mb-12'>
        <div className='flex flex-col md:flex-row gap-4 items-center justify-between p-4 bg-gray-50 rounded-[2rem] border border-gray-100'>
          <div className='relative w-full md:max-w-md group'>
            ...
            <input
              type='text'
              placeholder={t('categorySearchPlaceholder', { category: category.title })}
              className='w-full pl-14 pr-6 py-4 bg-white border border-gray-100 rounded-2xl focus:outline-none focus:ring-4 focus:ring-brand-purple/10 transition-all'
            />
          </div>
          <div className='flex items-center gap-3 w-full md:w-auto'>
            <button className='flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-4 bg-white border border-gray-100 rounded-2xl font-bold text-gray-600 hover:border-brand-purple hover:text-brand-purple transition-all'>
